...
def plot_multiple_curves(data_configs, node_id, output_path, start_frame=5, end_frame=824, 
                        dpi=300, title=None, xlabel='相对时间(s)', ylabel='实时占有率'):
    """绘制多条曲线的对比图表
    ...
    for config in data_configs:
        ...
        print(f"绘制 {label}: {len(df)} 个点, 时间范围 {df['time'].min()}-{df['time'].max()}, "
              f"占有率范围 {df['total_vehicles'].min():.2f}-{df['total_vehicles'].max():.2f}")
    ...
    if title is None:
        title = f'占有率特征变化曲线对比 (节点{node_id})'
...
def main():
    ...
    # 输出路径
    output_path = f'../data/draw/fintuned/node{NODE_ID}_num_compare.png'
