...
def plot_multiple_curves(data_configs, node_id, output_path, start_frame=5, end_frame=824, 
                        dpi=300, title=None, xlabel='相对时间(s)', ylabel='实时流量'):
    """绘制多条曲线的对比图表
    
    参数:
        data_configs: 列表，每个元素是一个字典，包含以下键：
            - 'data': DataFrame，包含time和total_vehicles列
            - 'label': str，图例标签
            - 'color': str，线条颜色
            - 'linestyle': str，线型 ('-', '--', '-.', ':')
    ...
    for config in data_configs:
        ...
        # 绘制曲线
        ax.plot(df['time'].values, df['total_vehicles'].values, 
                color=color, linestyle=linestyle, linewidth=linewidth, 
                alpha=alpha, label=label)
        
        print(f"绘制 {label}: {len(df)} 个点, 时间范围 {df['time'].min()}-{df['time'].max()}, "
              f"流量范围 {df['total_vehicles'].min():.2f}-{df['total_vehicles'].max():.2f}")
...
def main():
    ...
    # 输出路径
    output_path = f'../data/draw/d210191000/node{NODE_ID}_num_compare.png'
    ...
    '''
        {
            ...
            'linewidth': 1,
            'alpha': 0.5,
            'value_column': 'total_vehicles'  # 或 'interpolated_total_vehicles'
        },
    ...
    curve_configs = [
        ...
        },
        {
            ...
            'linewidth': 1,
            'alpha': 1.0,
            'value_column': 'total_vehicles'  # 数据列名
        }
    ]
    ...
    for config in curve_configs:
        ...
        try:
            ...
            # 处理列名（支持不同的列名）
            value_column = config.get('value_column', 'total_vehicles')
            
            # 如果指定的列不存在，尝试查找替代列
            if value_column not in node_data.columns:
                # 尝试常见的替代列名
                alternative_columns = ['interpolated_total_vehicles', 'total_vehicles', 'value']
                found = False
            ...
            # 重命名为统一的列名
            if value_column != 'total_vehicles':
                node_data['total_vehicles'] = node_data[value_column]
